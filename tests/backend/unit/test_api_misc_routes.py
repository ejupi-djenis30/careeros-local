...
def test_schedules_routes_full():
    ...
    with (
        patch("backend.api.routes.schedules.get_scheduler") as mock_get_scheduler,
        patch(
            "backend.api.routes.schedules.get_all_schedules", return_value=[{"id": 1}, {"id": 2}]
        ),
    ):
        # Scheduler running
...
def test_profiles_routes_full():
    ...
    mock_profile_service.get_profiles_by_user.return_value = [
        {
            "id": 1,
            "name": "hi",
    ...
    mock_profile_service.create_profile.return_value = {
        "id": 1,
        "name": "hi",
        "role_description": "dev",
        "location_filter": "us",
    ...
    # 2. create_profile
    client.post(
        "/api/v1/profiles/", json={"role_description": "dev", "location_filter": "us", "name": "hi"}
    )
