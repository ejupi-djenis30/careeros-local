...
from backend.api.deps import get_db, get_current_user_id
import io
...
def test_auth_refresh_user_vanished():
    ...
    with patch("backend.api.routes.auth.decode_refresh_token", return_value={"sub": "testuser"}), \
         patch("backend.api.routes.auth.get_db") as mock_db:
...
def test_search_upload_file_too_large():
    ...
    with patch("backend.api.routes.search.extract_text_from_file"):
        ...
        response = client.post("/api/v1/search/upload-cv", files={"file": ("test.pdf", large_file, "application/pdf")})
