...
class SourceApi:
    ...
    def json(self, _method: str, path: str) -> Any:
        ...
        if path == "/repos/owner/repo/git/ref/tags/v1.1.1":
...
def test_source_policy_requires_verified_annotated_tag_on_stable_default_branch() -> None:
    assert (
        verify_source_policy(
            SourceApi(), repo="owner/repo", tag="v1.1.1", source_commit=COMMIT
        )
        == "main"
    )
    ...
    with pytest.raises(RuntimeError, match="annotated"):
        verify_source_policy(
            SourceApi(annotated=False), repo="owner/repo", tag="v1.1.1", source_commit=COMMIT
        )
    with pytest.raises(RuntimeError, match="GitHub-verified"):
        verify_source_policy(
            SourceApi(verified=False), repo="owner/repo", tag="v1.1.1", source_commit=COMMIT
        )
    assert verify_source_policy(
        SourceApi(moving=True), repo="owner/repo", tag="v1.1.1", source_commit=COMMIT
    ) == "main"
    with pytest.raises(RuntimeError, match="identity changed"):
        verify_source_policy(
            SourceApi(renamed=True), repo="owner/repo", tag="v1.1.1", source_commit=COMMIT
        )
