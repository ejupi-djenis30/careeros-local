...
def test_source_policy_requires_verified_annotated_tag_on_stable_default_branch() -> None:
    assert (
        verify_source_policy(
            SourceApi(), repo="owner/repo", tag="v1.1.0", source_commit=COMMIT
        )
        == "main"
    )
    ...
    with pytest.raises(RuntimeError, match="annotated"):
        verify_source_policy(
            SourceApi(annotated=False), repo="owner/repo", tag="v1.1.0", source_commit=COMMIT
        )
    with pytest.raises(RuntimeError, match="GitHub-verified"):
        verify_source_policy(
            SourceApi(verified=False), repo="owner/repo", tag="v1.1.0", source_commit=COMMIT
        )
    assert verify_source_policy(
        SourceApi(moving=True), repo="owner/repo", tag="v1.1.0", source_commit=COMMIT
    ) == "main"
