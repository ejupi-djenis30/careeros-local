...
The editable install above registers `careeros` and `careeros-mcp` from this checkout. It relies on
the reviewed application dependency lock; do not run an unconstrained install to make the command
work. The desktop installer does not currently install either command on the system `PATH`.
Agent configuration generated by the CLI uses `careeros` as its command; run the client from the
activated environment or replace it with the absolute path to this environment's executable.
...
.venv\Scripts\python.exe -m ruff check backend/automation tests/backend/automation `
  alembic/versions/f2a3b4c5d6e7_add_automation_grants.py
