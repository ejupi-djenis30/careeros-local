    ...
    return (
        <main className="login-shell">
            ...
            <section className="login-panel" aria-labelledby="login-title">
                ...
                {sessionNotice && <div className="inline-alert inline-alert--warning" role="status">{t(sessionNotice)}</div>}
                ...
                <form onSubmit={submit}>
                    <label className="field-stack"><span>{t("login.username")}</span><div className="input-with-icon"><i className="bi bi-person" /><input className="form-control" name="username" value={username} onChange={(e) => setUsername(e.target.value)} required autoFocus autoComplete="username" /></div></label>
                    <label className="field-stack"><span>{t("login.password")}</span><div className="input-with-icon"><i className="bi bi-key" /><input className="form-control" name="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required maxLength={72} autoComplete={mode === "register" ? "new-password" : "current-password"} /></div></label>
