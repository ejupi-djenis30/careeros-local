    ...
    return (
        <div className="home-grid">
            ...
            <section className="metric-grid" aria-label={t("home.summary")}>
                ...
                <Link to="/jobs" className="metric-card metric-card--action"><i className="bi bi-radar" /><span>Opportunity engine</span><strong>{t("home.open")}</strong><small>{t("home.localSearch")}</small></Link>
            </section>
            ...
            <section className="surface-section home-activity"><div className="section-heading"><div><span className="section-kicker">Pipeline</span><h2>{t("home.recentActivity")}</h2></div><Link to="/applications">{t("home.viewAll")}</Link></div>{recent.length ? <div className="recent-list">{recent.map((application) => <Link key={application.id} to="/applications"><span className={`stage-dot stage-dot--${application.current_stage}`} /><div><strong>{application.title}</strong><small>{application.company}</small></div><span>{stageLabels[application.current_stage]}</span></Link>)}</div> : <div className="empty-inline"><p>{t("home.noApplications")}</p></div>}</section>

            <section className="surface-section home-runtime"><div className="section-heading"><div><span className="section-kicker">Runtime</span><h2>{t("home.onDeviceModel")}</h2></div></div><ModelManager /><div className="local-architecture"><span>Career Vault</span><i className="bi bi-arrow-right" /><span>{t("home.chosenContext")}</span><i className="bi bi-arrow-right" /><span>{t("home.localLlama")}</span></div><p>{t("home.modelDisclosure")}</p></section>
