...
export function ApplicationDetail({ application, resumeVersions = [], resumeMetadataStatus = "ready", onRetryResumeMetadata, onChanged, onClose, dialogRef }) {
    ...
    const eventLabels = Object.fromEntries(["stage", "note", "task", "contact", "interview", "preparation", "task_created", "task_updated", "task_completed", "task_reopened", "task_cancelled", "dossier_published"].map((type) => [type, t(`applicationEvent.${type}`)]));
        ...
        <div ref={dialogRef} className="application-detail" role="dialog" aria-modal="true" aria-labelledby="application-detail-title" aria-describedby="application-detail-summary" tabIndex="-1">
            ...
            <form className="application-event-form" onSubmit={submit}>
                ...
                <div className="form-grid form-grid--2">
                    <label className="field-stack"><span>{t("applicationDetail.eventType")}</span><select className="form-select" value={activeEventType} onChange={(e) => { setEventType(e.target.value); if (e.target.value === "stage") setStage(availableTransitions[0] || ""); }}>{["stage", "note", "contact", "interview"].filter((value) => value !== "stage" || availableTransitions.length).map((value) => <option key={value} value={value}>{eventLabels[value]}</option>)}</select></label>
                ...
                <label className="field-stack"><span>{t("applicationDetail.note")}</span><textarea className="form-control" rows="3" value={note} onChange={(e) => setNote(e.target.value)} required={["note", "contact", "interview"].includes(activeEventType)} placeholder={t("applicationDetail.placeholder")} /></label>
