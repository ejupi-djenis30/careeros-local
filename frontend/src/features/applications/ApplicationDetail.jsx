...
export function ApplicationDetail({ application, onChanged, onClose }) {
    ...
    const eventLabels = Object.fromEntries(["stage", "note", "task", "contact", "interview"].map((type) => [type, t(`applicationEvent.${type}`)]));
        ...
        <aside className="application-detail" aria-labelledby="application-detail-title">
            ...
            <form className="application-event-form" onSubmit={submit}>
                ...
                <div className="form-grid form-grid--2">
                    <label className="field-stack"><span>{t("applicationDetail.eventType")}</span><select className="form-select" value={eventType} onChange={(e) => { setEventType(e.target.value); if (e.target.value === "stage") setStage(TRANSITIONS[application.current_stage][0] || ""); }}>{Object.entries(eventLabels).filter(([value]) => value !== "stage" || TRANSITIONS[application.current_stage].length).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
                    {eventType === "stage" && <label className="field-stack"><span>{t("applicationDetail.newStage")}</span><select className="form-select" value={stage} onChange={(e) => setStage(e.target.value)}>{TRANSITIONS[application.current_stage].map((value) => <option key={value} value={value}>{stageLabels[value]}</option>)}</select></label>}
                </div>
                <label className="field-stack"><span>{eventType === "task" ? t("applicationDetail.activity") : t("applicationDetail.note")}</span><textarea className="form-control" rows="3" value={note} onChange={(e) => setNote(e.target.value)} required={["note", "contact", "interview"].includes(eventType)} placeholder={t("applicationDetail.placeholder")} /></label>
                <button className="button button--primary" disabled={busy || (eventType === "stage" && !stage)}>{busy ? t("applicationDetail.recording") : t("applicationDetail.record")}</button>
            </form>
