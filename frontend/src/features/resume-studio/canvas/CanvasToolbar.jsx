...
export function CanvasToolbar({ state, templateKind, dispatch, onAddClaim, zoom, onZoom, pageCount }) {
    ...
    return (
        <div className="canvas-toolbar" aria-label={t("canvas.tools")}>
            ...
            <label><span>{t("canvas.font")}</span><select value={style.font_family} onChange={(event) => setStyle("font_family", event.target.value)}><option>Helvetica</option><option>Arial</option><option>Georgia</option></select></label>
