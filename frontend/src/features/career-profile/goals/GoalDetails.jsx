...
export function GoalDetails({ payload, onChange }) {
    ...
    return (
        <div className="goal-details">
            ...
            <div className="form-grid form-grid--3">
                <ListField label={t("goal.targetRoles")} value={payload.target_roles} onChange={(value) => update("target_roles", value)} placeholder="Staff Engineer, Engineering Manager" />
                <ListField label={t("goal.targetIndustries")} value={payload.target_industries} onChange={(value) => update("target_industries", value)} placeholder="Software, Fintech" />
                <ListField label={t("goal.targetLocations")} value={payload.target_locations} onChange={(value) => update("target_locations", value)} placeholder="Zurich, remote" />
                <ListField label={t("goal.targetSeniority")} value={payload.target_seniority} onChange={(value) => update("target_seniority", value)} placeholder="senior, staff, lead" />
                <ListField label={t("goal.workModes")} value={payload.work_modes} onChange={(value) => update("work_modes", value)} placeholder="onsite, hybrid, remote" />
                <ListField label={t("goal.contractTypes")} value={payload.contract_types} onChange={(value) => update("contract_types", value)} placeholder="permanent, contract" />
            </div>
            ...
            <div className="form-grid form-grid--3"><ListField label={t("goal.successCriteria")} value={payload.success_criteria} onChange={(value) => update("success_criteria", value)} placeholder="Signed offer, aligned role" /><ListField label={t("goal.mustHaves")} value={payload.must_haves} onChange={(value) => update("must_haves", value)} /><ListField label={t("goal.dealBreakers")} value={payload.deal_breakers} onChange={(value) => update("deal_breakers", value)} /></div>
        </div>
    );
}
