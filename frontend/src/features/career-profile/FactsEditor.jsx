        ...
        <section className="surface-section" aria-labelledby="facts-title">
            ...
            <div className="fact-list">
                {facts.length === 0 ? <div className="empty-inline"><p>Aggiungi esperienze, competenze e risultati: diventano la fonte unica per ogni documento.</p></div> : facts.map((fact, index) => (
                    <FactEditor key={fact.id || fact.clientKey} fact={fact} index={index} total={facts.length} evidenceOptions={facts.filter((item) => item.id && item.id !== fact.id).map((item) => ({ id: item.id, label: factTitle(item), type: FACT_TYPES.find(([value]) => value === item.fact_type)?.[1] || item.fact_type }))} evidenceState={analysis?.evidence?.find((item) => item.fact_id === fact.id)} onChange={(next) => update(index, next)} onRemove={() => onChange(facts.filter((_, position) => position !== index))} onMove={move} />
                ))}
            </div>
        </section>
    );
