...
def compute_prescore(
    ...
    domain_affinity: Callable[[str, str], float] | None
    try:
        from backend.data.domain_affinity import get_domain_affinity

        domain_affinity = get_domain_affinity
    except Exception:
        ...
        for pd in profile_domains:
            ...
            if pd_str:
                if domain_affinity is not None:
                    aff = domain_affinity(job_domain, pd_str)
                else:
