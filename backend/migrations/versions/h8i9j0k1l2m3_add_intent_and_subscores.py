...
def upgrade() -> None:
    ...
    op.add_column(
        "search_profiles", sa.Column("profile_search_intent_domain", sa.String(), nullable=True)
    )
    op.add_column(
        "search_profiles", sa.Column("profile_search_intent_seniority", sa.String(), nullable=True)
    )
    op.add_column(
        "search_profiles",
        sa.Column("profile_search_intent_role_family", sa.String(), nullable=True),
    )
    op.add_column(
        "search_profiles",
        sa.Column("profile_search_intent_qualification_level", sa.String(), nullable=True),
    )
    op.add_column(
        "search_profiles", sa.Column("profile_search_intent_skills", sa.JSON(), nullable=True)
    )
    op.add_column(
        "search_profiles",
        sa.Column(
            "profile_search_intent_open_to_unrelated",
            sa.Boolean(),
            nullable=True,
    ...
    op.add_column(
        "search_profiles", sa.Column("profile_search_intent_keywords", sa.JSON(), nullable=True)
    )

    # ── scraped_jobs: industry sector + role type ─────────────────────────
    op.add_column(
        "scraped_jobs", sa.Column("normalized_industry_sector", sa.String(), nullable=True)
    )
    op.add_column("scraped_jobs", sa.Column("normalized_role_type", sa.String(), nullable=True))
    op.create_index(
        "ix_scraped_jobs_normalized_role_type", "scraped_jobs", ["normalized_role_type"]
    )
