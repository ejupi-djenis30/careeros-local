...
def upgrade() -> None:
    ...
    op.create_index("ix_jobs_user_profile", "jobs", ["user_id", "search_profile_id"], unique=False)

    # Composite index for jobs sorted by score (most common sort in job listing)
    op.create_index("ix_jobs_user_score", "jobs", ["user_id", "affinity_score"], unique=False)
