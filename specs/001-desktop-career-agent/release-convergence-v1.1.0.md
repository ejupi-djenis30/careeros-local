...
| FR-038 exact candidate | `release_assets.py` and `release_contract.py` normalize six targets, reject missing/extra targets, empty files/links/unsafe names/casefold collisions, and bind target/type/name/size/SHA-256, exact checksum names, a first-class canonical MIT `LICENSE`, three SBOMs, and deterministic evidence into 23 public assets. Tauri embeds the same license bytes, and platform smoke gates inspect mounted, extracted, or installed MSI, NSIS, AppImage, DEB, and DMG payloads before staging. | Candidate and license tamper/omission/duplication, canonical-newline, lowercase-duplicate and symlink-alias rejection, target coverage, empty package, collision, duplicate-type, native-license path, mounted-DMG lifecycle/detach, missing/foreign evidence, reproducibility, exact inventory, and checksum tests. | Converged |
