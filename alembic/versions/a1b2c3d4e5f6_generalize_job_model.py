...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE jobs SET external_url = jobroom_url WHERE external_url IS NULL OR external_url = ''"
    )
    ...
    op.add_column(
        "search_profiles", sa.Column("is_history", sa.Boolean(), server_default="0", nullable=False)
    )
    op.add_column(
        "search_profiles", sa.Column("is_stopped", sa.Boolean(), server_default="0", nullable=False)
    )
    op.add_column(
        "search_profiles",
        sa.Column(
