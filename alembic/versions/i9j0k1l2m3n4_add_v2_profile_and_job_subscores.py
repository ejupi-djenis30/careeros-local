...
def upgrade() -> None:
    ...
    op.add_column(
        "search_profiles", sa.Column("profile_normalized_role_type", sa.String(), nullable=True)
    )
    op.add_column(
        "search_profiles",
        sa.Column("profile_normalized_industry_sectors", sa.JSON(), nullable=True),
    )
    op.add_column(
        "search_profiles",
        sa.Column("profile_normalized_transferable_skills", sa.JSON(), nullable=True),
    )

    # ── search_profiles: V2 enhanced search intent ────────────────────────
    op.add_column(
        "search_profiles", sa.Column("profile_search_intent_role_type", sa.String(), nullable=True)
    )
    op.add_column(
        "search_profiles",
        sa.Column("profile_search_intent_seniority_min", sa.String(), nullable=True),
    )
    op.add_column(
        "search_profiles",
        sa.Column("profile_search_intent_seniority_max", sa.String(), nullable=True),
    )
    op.add_column(
        "search_profiles", sa.Column("profile_search_intent_dealbreakers", sa.JSON(), nullable=True)
    )
    op.add_column(
        "search_profiles", sa.Column("profile_search_intent_flexibility", sa.JSON(), nullable=True)
    )
